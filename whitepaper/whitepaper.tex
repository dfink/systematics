...
\begin{figure}[t]
  \begin{center}
    \includegraphics[width=0.5\linewidth]{plots/option3v3_lineEval.eps}
    \vspace{-0.5cm}
  \end{center}
...
The upper panel of Fig.~\ref{fig:mollweide} shows the exposure map after 55
days of mixed observation mode in equatorial coordinates. It is normalized to
the mean exposure in standard survey mode
($\mathcal{E}_0=4.1\times10^9\rm\,cm^2s$ after 55 days). The point of highest
exposure is at (RA, Dec)$\approx(260^\circ$, $0^\circ)$, which is displaced
from the GC by $29^\circ$. Directly at the GC, the exposure increases by a
factor of XXX with respoct to normal survey mode.
...
fact slightly reduced w.r.t.~standard survey mode, i.e.~$\Delta E/E=XXX$
instead of $\Delta E/E=XXX$.

As \emph{figure-of-merit} for line searches we define the dimensionless
quantity $Q\equiv\sqrt{\mathcal{E}/\Delta E}/Q_0$, which is proportional to
the average significance expected for a line signal, and $Q_0$ is an arbitrary
normalization. A sky map of $Q_m/Q_s$ is shown in the bottom panel of
Fig.~\ref{fig:mollweide}, where $Q_m$ referes to the mixed and $Q_s$ to the
standard survey mode, respectively. Regions close to the GC have values of
$Q_m/Q_s=XXX$. This is the factor by which the growth of a gamma-ray line
signal at the GC would be accelerated by switching from survey to mixed
observation mode.
...
whereas is spans a factor $12$ in the mixed observation mode. However, only in
a small fraction of the sky, about XXX\%, the coverage drops by as much as a
factor of $0.3$--$0.5$, and never below $0.3$.

Figs.~\ref{fig:expHisto} and~\ref{fig:mollweide} show the exposure integrated
over the orbital precession period of 55 days. However, for the observation of
transient phenomena, it is vital that all parts of the sky are sufficiently
covered at least each day. In Fig.~\ref{fig:coverage} we show for each
individual day of the precession period which fraction of the sky are covered
by different amounts of exposure. \dots
...
\begin{figure}[t]
  ...
  55 days to complete on orbit precession period).} %Right panel: in stead the sky
