...
Finkbeiner~\citep{linepaper} approached the problem with template fitting, which takes into
...
Einasto profile centered $1.5\degree$ west of the Galactic center, and found
that there may be two lines, at about 111 and 129
GeV (as earlier pointed out in Ref.~\cite{Rajaraman:2012}). The lower
...
Kyae:2012, Lee:2012, Rajaraman:2012, Acharya:2012, Ibarra:2012,
...
less significant~\cite{Albert:Talk}. Throughout, we will refer to the feature
as `130~GeV feature' for simplicity.
...
draw strong conclusions from this single observation.
...
\begin{figure}[t]
  ...
  system; sky maps are symmetric around the celestial equator; color scales
  are linear.}
  \label{fig:mollweide}
\end{figure}
...
once the target reenters $10^\circ$ from Earth occultation.
...
\begin{figure}[t]
  ...
  \caption{Histogram of exposure per sky pixel, normalized to standard survey
    ...
    shows the modified strategy option2v2 and option3v3.}
  \label{fig:expHisto}
\end{figure}
...
number of sources with $>150\%$ exposure, but a 50\% (33\%)
...
technically possible would be suggested as response to this valuable chance
...
\item{\it Fermi can do it: a modified survey strategy can obtain a decisive
  ...
  the signal strength broaden this range such that the actual significance
...
MS and CW thank the \emph{Kavli Institute for Theoretical Physics} in Santa
Barbara, California, for kind hospitality during the final stages of this
work.
