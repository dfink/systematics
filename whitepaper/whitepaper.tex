...
to the $\pm1\sigma$ flux uncertainties from the 3.5 year results. We checked
that similar results are obtained for Region 4, or in the hourglass region
discussed below.
...
center, focusing on the mixed mode ``option3v3'' put forth for discussion by
the Fermi mission.\footnote{See
...
strategy would be to switch to pointed observation of the Galactic center when
possible, and to follow the standard survey mode
otherwise. More precisely, \Fermi\ would slew from survey mode to the target
once the target is $10^\circ$ from Earth occultation, and slew back to survey
mode position once the target reenters $10^\circ$ from Earth occultation.
...
the range Dec=$\pm25.6^\circ$ during one orbital precession period such that the
...
fact slightly worsen than in standard survey mode ($\Delta E/E=9.59\%$ instead
of $\Delta E/E=8.75\%$).  This loss of resolution has only a small effect on
line searches.
...
average expected line significance in units of Gaussian sigma. Here, $a$
normalizes $Q$ such that the spatial mean in survey mode
...
are collected at lower incidence angles. This would accelerate the accumulation
of low incidence angle Earth limb data with respect to the previous 4.5 years by about
...
\begin{figure}[t]
  \begin{center}
    ...
    \vspace{-0.5cm}
  \end{center}
  \caption{Histogram of fractional exposure per sky pixel, normalized to
  ...
  strategy option 1 relative to option 3. {\bf two panels??}}
  \label{fig:expHisto}
\end{figure}
