...
\begin{figure}[h]
  ...
  \caption{
    ...
    signal rate is $1.6\pm 0.4$ events per month, the effective background is
    $4.6$ events per month (the values measured prior to 4 February 2012).
    The second and third vertical dotted lines indicate how much exposure is
...
data: on average $N_s=9.3\pm2.4$ events per bin, with
...
increases by a factor of 2.2 with respect to normal survey mode.
...
fact slightly worsen w.r.t.~standard survey mode: namely $\Delta E/E=9.4\%$
instead of $\Delta E/E=8.4\%$. However, this has only a small effect on line
searches.
...
by a factor $1.39$ when switching from survey to mixed mode. Hence, for
