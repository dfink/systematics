...
first identified. We use this region to avoid introducing additional trials,
or making assumptions about the underlying signal profile. Below we will
...
information in the modeling of the line.} We find a number of $N_s=XXX\pm XXX$
excess events (with $\pm1\sigma$ profile likelihood errors), with a
statistical significance of $s=XXX$ standard deviations. The effective number
of background events can estimated as $N_b = N_s^2/s^2$, and yields $N_b=XXX$.
...
obtain $N_s$ and $N_b$, we fit the data from the six month intervals
individually. The gray bands show the number of signal events expected from
the above fit to 3.5 years of data; this is on average $N_s=7.1\pm 1.8$ events
per bin, with mild variations being related to the exposure. 
...
represent a decent downward fluctuation. However, it is still too premature to
draw strong conclusions from that.
\medskip

\emph{Projection.} In Fig.~2 we shows 68\% and 95\% CL bands for the projected
evolution of the signal significance as function of exposure accumulated after
4 February 2012.  To generate the plot, we simulate events from a power-law
plus line model and derive the signal significance for each realization as
described above. The background and signal correspond to the best-fit values
obtained in Region 3 with data until 4 February 2012.  For different
realizations we allow the signal normalization to vary following a normal
distribution that is matched to the $\pm1\sigma$ flux uncertainties from the
3.5 year results. We checked that similar results are obtained for Region 4,
or in the hourglass region discussed below.
...
2012. We define is in $(\ell, b)$ space as the intersection of $\ell^2+b^2\leq
r^2$ and $\ell^2\leq (b\tan\varphi)^2 + d^2$, with $(r, d, \varphi) =
(20^\circ, 3^\circ, 60^\circ)$, and has similar signal-to-noise properties as
Region 3 or 4. 
...
\begin{figure}[t]
  \begin{center}
    \includegraphics[width=0.49\linewidth, angle=-90]{plots/exposure_pixel_hist_perday_survey.ps}
    \vspace{-0.5cm}
  \end{center}
  \caption{The spatial distribution of FAVA sources overplotted with the exposure map of Option 1 survey strategy. Red circles mark sources with expected exposure $>150\%$ than the standard survey strategy with a total number of 25 sources. Green circles mark sources with expected exposure $<50\%$ than the standard survey strategy with a total number of 59 sources. The rest sources are marked by blue cycles in total 131. }
  \label{fig:expHisto}
\end{figure}
