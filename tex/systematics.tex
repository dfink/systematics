...
%% (typical incident angles, brightness, hardness of spectra etc), as well as the
%% "from the top" plots that I mentioned in the previous mail. In these plots, I
%% show the significance for a 130 GeV features at different incident angles of
%% the detector. The TS values are calculated in the usual way (although this
%% time I approximate the bump by a simple Gaussian). I also plot the TS value as
%% function of energy. The basic message is that there is nothing special about
%% theta = 30 - 45 deg and E = 130 GeV, although there is some weak indication
%% for larger TS values when one only includes z>110 events in the game. This
%% does not really look worrisome, since there is nothing at z<100.
...
% We checked that our results remain qualitatively the same when using instead
% \texttt{P7SOURCE\_V6} events. \textbf{(check!)}
