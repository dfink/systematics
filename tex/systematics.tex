...
tests.  Cosmic rays (CR) entering the Earth's atmosphere nearly parallel to
the ground initiate cascades, producing a featureless gamma-ray spectrum on
the horizon as viewed from \Fermi~\citep[e.g.][and references
therein]{FermiLimb}.  These ``Earth limb'' photons (also called ``albedo''
photons) are created in a highly boosted frame, and therefore have a smooth
spectrum extending up to $E \gg 130$ GeV. 
...
With a field of view of approximately 2 sr, the LAT can survey the entire sky
...
This rocking profile, combined with the precession
of the orbit every $\sim53.4$ days, allows the LAT to cover
the whole sky with approximately uniform coverage.

Survey mode is only occasionally interrupted for
pointed observations of targets of opportunity (ToOs). Pointed 
observations account for $<$20\%
of the total observing time. {\bf I thought it was even much less} \Fermi's survey observations are
...
i.e. the solar panels face the $+x$ direction.   $\phi$ is the
incidence angle with respect to the $+x$ axis.  {\bf I think this was wrong
  before and fixed it -- make sure you agree -- DPF}
...
In Table~\ref{tab:regions} we define some key sky and
...
\begin{figure}[h]
  ...
  Table~\ref{tab:regions} `Earth limb line'), as function of
...
Table~\ref{tab:regions}. Another `region' with very high
...
\bf{[We might want to move this paragraph somewhere
    else]}{\emph{We emphasize that signal significances
    calculated from the GC region as adopted in this paper
    are not sufficient to represent the full significance of
    the putative signal, which is usually higher when
    adopting regions with optimized signal-to-noise
    ratio~\cite{Weniger:2012} or a template
    analysis~\cite{linepaper}. We use the GC region here
    since it should be dominated by line events, making it a
    good starting point to look for suspicious trends in the
    data.}}
...
In Table~\ref{tab:regions}, we list the ratio of $>300\GeV$
...
The gamma-ray emission from Earth is produced by CR (mostly
protons, helium, and heavier nuclei) interactions with
atmosphere material. Such earth limb gamma rays at $\sim$130
GeV are mainly from the decay of neutral pions and kaons,
and the cross section are peaked in the forward direction
simply due to kinetics.  In this section, we follow the weak
indication of the limb photon excess at $\sim$130 GeV and
analyze the limb photons in more detail. We show that the
critical incident angles are not (solely) responsible for
the GC excess.
