...
Aug 4th 2008 to Jul 18th 2012, with the good-time-interval cuts
...
However, besides the Galactic center (with a flux of $\sim 8\times10^{-8}
\ph\cm^{-2}\s^{-1}\sr^{-1}$ above 100 GeV) a few other bright regions
exist, and we check as a warm-up whether they show indications for
...
Table~\ref{tab:regions}. All these samples provide a large number of events;
...
`hotspots' at other regions of the sky; they could be related to instrumental
effects or an indication for an exotic source population.
...
concentrated near the GC~\cite{Boyarsky:2012ca}. 
In the absence of knowledge about the distribution of such
...
45\degree$, as indicated in Fig.~\ref{fig:polarPlotsAll}
(this $\theta$ range contains about 5\% of the limb
events). No significant excess in this energy range has been
found in other $\theta$ regions (see
...
GeV excess appears equally in \texttt{FRONT} and \texttt{FRONT} converting
