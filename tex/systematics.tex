...
\begin{figure}[h]
  \begin{center}
    ...
    \vspace{-0.5cm}
  \end{center}
  \caption{Distribution of incidence angles for all $E>100\GeV$ events (blue
  dots) and for the GC line events (red dots, see text for definition) in the
  ($\theta$, $\phi$) plane. The contribution from
  the earth limb is clearly visible at $\theta\gtrsim 60^\circ$.}
...
Event selection: XXX months of data, SOURCE (??) events,
\texttt{DATA\_QUAL==1} and \texttt{LAT\_CONFIG==1} cuts (but leave
\texttt{ROCK\_ANGLE} unconstrained). \textbf{(update!)}
...
\begin{figure*}
  ...
  distribution of GC events as function of time modulo one year (with January 1st
  as origin). In December (June) the Sun stands in Sagittarius (anti galactic
  ...
  fixed to the Sun to keep the solar panels aligned. \emph{Lower panels:} PHI
